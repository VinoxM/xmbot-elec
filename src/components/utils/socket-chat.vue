<template>
  <div v-loading="loading">
    <el-row class="text-center chat-header">
      ...
      <el-dropdown style="position: absolute;right: 10px;top: 5px">
        <span :class="apiStatus.ready?'cur-pointer':'cur-not-allowed'">Api状态({{apiStatus.bot.length}})<i v-if="apiStatus.ready" class="el-icon-arrow-down el-icon--right"></i></span>
        <el-dropdown-menu slot="dropdown" v-if="apiStatus.ready">
          <el-dropdown-item v-for="(v,k,i) of apiStatus.api">
            <span>{{k}}: </span>
            <el-tag :type="v?'success':'info'" size="small">{{v?'已启动':'未启动'}}</el-tag>
          </el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    </el-row>
...
export default {
  ...
  computed: {
    ...
    apiStatus() {
      return this.$store.state.botReady
    }
  },
  watch: {
    ws_(val) {
